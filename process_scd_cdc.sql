...
BEGIN
    -- 1. Full reprocess → truncate everything
    IF (v_last_pos = 0) THEN
        TRUNCATE TABLE product_status_hst;
    END IF;

    -- 2. New CDC records (deduplicated)
    CREATE OR REPLACE TEMP TABLE new_cdc AS
    WITH dedup AS (
        SELECT *,
               ROW_NUMBER() OVER (PARTITION BY product_key, status, change_type, change_time, cdc_log_position) AS rn
    ...
    -- 4. Delete old SCD rows for affected products (incremental safety)
    ...
    CREATE OR REPLACE TEMP TABLE product_cdc AS
    SELECT 
        product_key,
        status,
        PARSE_JSON(change_type):type::STRING AS change_type,
    ...
    -- Ordered changes + flag start/close events
    CREATE OR REPLACE TEMP TABLE flagged AS
    WITH ordered_cdc AS (
        SELECT *,
               ROW_NUMBER() OVER (PARTITION BY product_key ORDER BY change_time ASC, cdc_log_position ASC) AS seq
        FROM product_cdc
    )
    SELECT
        product_key,
        ...
        (change_type = 'INSERT' OR 
         (change_type = 'UPDATE' AND (prev_status IS NULL OR status != prev_status OR prev_change_type = 'DELETE'))) AS is_start
    FROM ordered_cdc;

    -- Version starts (these are the rows we will insert)
    ...
    USING (
        ...
        LEFT JOIN (
            SELECT product_key, change_time AS close_time
            FROM flagged
            WHERE change_type IN ('DELETE', 'UPDATE') 
    ...
    ON 1 = 0   -- pure insert (no matching needed)
    ...
    RETURN 'SCD Type 2 processed successfully using MERGE. Rows inserted: ' || 
           (SELECT COUNT(*) FROM version_starts);
END;
$$;
